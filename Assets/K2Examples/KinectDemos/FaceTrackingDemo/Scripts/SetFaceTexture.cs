...
public class SetFaceTexture : MonoBehaviour 
{
    ...
    //private Texture2D foregroundTex2d = null;
    private bool bSkipUpdatingForegroundTex = false;

    private Vector3 colorScale = Vector3.one;
	...
	void Start () 
	{
        ...
        colorScale = kinectManager.GetColorImageScale();
	...
	void Update () 
	{
		...
		if (!bSkipUpdatingForegroundTex && backManager && backManager.IsBackgroundRemovalInitialized()) 
		{
			...
			if (bmForeTex is RenderTexture) 
			{
                ...
                //foregroundTex2d = new Texture2D(bmForeTex.width, bmForeTex.height, TextureFormat.ARGB32, false);
...
//			}
		...
		if (faceRect.width > 0 && faceRect.height > 0) 
		{
            ...
            if (foregroundTex) 
			{
				KinectInterop.RenderTex2Tex2D(foregroundTex, faceX, faceY /**foregroundTex.height - faceY - faceH*/, faceW, faceH, ref faceTex);
    //            KinectInterop.RenderTex2Tex2D(foregroundTex, ref foregroundTex2d);

				//byte[] btColorPng = foregroundTex2d.EncodeToPNG();
				//System.IO.File.WriteAllBytes("_ForegroundTex.png", btColorPng);

				//byte[] btFacePng = faceTex.EncodeToPNG();
				//System.IO.File.WriteAllBytes("_FaceTex.png", btFacePng);

				//string faceText = $"face x: {faceX}, y: {faceY}, w: {faceW}, h: {faceH}\nfgTex w: {foregroundTex2d.width}, h: {foregroundTex2d.height}";
				//System.IO.File.WriteAllText("_FaceRect.txt", faceText);
			}
            else if(colorTex)
			{
				...
				//byte[] btColorPng = colorTex.EncodeToPNG();
	            //System.IO.File.WriteAllBytes("_ColorTex.png", btColorPng);

				//byte[] btFacePng = faceTex.EncodeToPNG();
                //System.IO.File.WriteAllBytes("_FaceTex.png", btFacePng);

				//string faceText = $"face x: {faceX}, y: {faceY}, w: {faceW}, h: {faceH}\nclrTex w: {colorTex.width}, h: {colorTex.height}";
				//System.IO.File.WriteAllText("_FaceRect.txt", faceText);
			}
